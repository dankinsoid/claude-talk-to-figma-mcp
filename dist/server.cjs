...
var saveParams = {
  saveToFile: import_zod.z.boolean().optional().describe(
  ...
  outputPath: import_zod.z.string().optional().describe(
...
var shapeParams = {
  depth: import_zod.z.number().int().min(0).optional().describe("Levels of children below the requested node to expand (default 6). Deeper nodes become stubs with {childCount, more:true}; re-request that id to zoom in. Raise to see more at once, lower for a terser overview."),
  collapseIcons: import_zod.z.boolean().optional().describe("Collapse icon-like subtrees (no text, vector leaves) to a single ICON node with more:true (default true)."),
  collapseRepeats: import_zod.z.boolean().optional().describe("Collapse repeated instances of the same component: the first renders in full, later copies become a stub with their props/text and more:true (default true)."),
  cull: import_zod.z.boolean().optional().describe("Drop nodes that render nowhere \u2014 fully clipped out by an ancestor's clipsContent ({id, clipped:true}) or fully covered by an opaque sibling above ({id, occluded:true}). Default true.")
};
server.tool(
  "read_my_design",
...
server.tool(
  ...
  {
    pattern: import_zod.z.string().describe('JavaScript regular expression source (NOT a shell glob). E.g. "\\\\bCTA\\\\b", "\\\\$\\\\d+", "left$".'),
    root: import_zod.z.string().optional().describe("Node id to search under. Defaults to the current page. Accepts short ids (n0, ...)."),
    ignoreCase: import_zod.z.boolean().optional().describe("Case-insensitive match (regex `i` flag). Default false."),
    onlyMatch: import_zod.z.boolean().optional().describe("Report only the matched substring(s) per line instead of the whole line (grep -o). Default false."),
    mode: import_zod.z.enum(["content", "nodes", "count"]).optional().describe('Output shape: "content" (default; each matching line), "nodes" (one line per matching node + its hit count), or "count" (totals only).'),
    depth: import_zod.z.number().optional().describe("Max depth below root to descend (root's direct children = 1). Omit for unlimited."),
    within: import_zod.z.object({
      x: import_zod.z.number(),
      y: import_zod.z.number(),
    ...
    bbox: import_zod.z.boolean().optional().describe("Append each hit node's absolute bounding box as [x,y wxh]. Default false."),
    maxMatches: import_zod.z.number().optional().describe("Hard cap on collected line-hits before the walk stops. Default 1000."),
    ...saveParams
  },
...
server.tool(
  ...
  {
    where: import_zod.z.array(
      import_zod.z.object({
        path: import_zod.z.string().describe('Field path on the node. Dot for objects, [i] for an index, [*] for any array element. E.g. "fontSize", "fills[*].color", "boundVariables.fills", "name".'),
        op: import_zod.z.enum(["regex", "gt", "gte", "lt", "lte", "color", "exists", "absent"]).optional().describe('Match op. Default "regex". Use gt/gte/lt/lte for numbers, color for #RRGGBB, exists/absent for key presence (no value needed).'),
        value: import_zod.z.union([import_zod.z.string(), import_zod.z.number(), import_zod.z.boolean()]).optional().describe('Comparison value. Regex source for "regex", a number for compares, "#RRGGBB" for color. Omit for exists/absent.'),
        i: import_zod.z.boolean().optional().describe('Case-insensitive regex (only for op "regex"). Default false.')
      })
    ...
    bbox: import_zod.z.boolean().optional().describe("Append each hit's absolute bounding box as [x,y wxh]. Default false."),
    maxMatches: import_zod.z.number().optional().describe("Hard cap on collected hits before the walk stops. Default 1000."),
    ...saveParams
  },
