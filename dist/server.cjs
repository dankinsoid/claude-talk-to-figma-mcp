...
var shapeParams = {
  detail: import_zod.z.enum(["skeleton", "box", "style", "text", "auto", "full"]).optional().describe(
  ...
  maxNodes: import_zod.z.number().int().min(1).optional().describe("Soft node budget (default 100). The tree expands breadth-first until ~this many nodes are emitted; the rest become stubs with {childCount, more:true}. Raise to see more at once, lower for a terser overview."),
  depth: import_zod.z.number().int().min(0).optional().describe("Optional hard cap on levels of children (default: unbounded \u2014 the node budget governs). Stubbed nodes carry {childCount, more:true}; re-request that id to zoom in."),
  ...
  dedupe: import_zod.z.boolean().optional().describe("Hoist repeated fills/strokes into a shared _defs table referenced by @N strings (default true; only applies when paints are included).")
};
server.tool(
  "read_my_design",
...
server.tool(
  ...
  async ({ nodeId, format, scale, outputPath, saveToFile }) => {
    try {
