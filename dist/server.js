...
import { tmpdir as tmpdir3, homedir } from "os";
...
var DIR = join(tmpdir(), "talk-to-figma", "idmap");
...
var ACTIVE_CHANNELS_FILE = join2(tmpdir2(), "figma-active-channels.json");
...
async function fetchAssetBytes(url) {
  ...
  else if (url.startsWith("~/")) localPath = join3(homedir(), url.slice(2));
  let buf;
  if (localPath !== void 0) {
...
async function writeOutputFile(baseName, ext, data, outputPath) {
  ...
  const target = outputPath ? outputPath : join3(tmpdir3(), "talk-to-figma", `${safeBase}-${Date.now()}-${outputFileSeq++}.${ext}`);
...
server.tool(
  ...
  {
    nodeId: z4.string().optional().describe("Source node id, single form. Short ids n0,... or full Figma ids."),
    x: z4.number().optional().describe("Absolute canvas X (single form)."),
    y: z4.number().optional().describe("Absolute canvas Y (single form)."),
    clones: z4.array(
      z4.object({
        nodeId: z4.string().describe("Source node id (short n0,... or full Figma id)."),
        x: z4.number().optional().describe("Absolute canvas X, as glob_nodes reports. Ignored if the parent is auto-layout; omit to stack on the source."),
        y: z4.number().optional().describe("Absolute canvas Y.")
      })
...
server.tool(
  ...
  "Export one or more nodes as images from Figma. Each node may request several scales at once, so a single call can produce many files. To actually look at how a node renders, pass inline:true with a small scale (e.g. 0.5) so the image comes back in the response and stays cheap on context.",
  {
    ...
    contentsOnly: z4.boolean().optional().describe(
    ...
    useAbsoluteBounds: z4.boolean().optional().describe(
    ...
    for (const { nodeId, scale } of nodes) {
      try {
        ...
        for (const img of typedResult.images) {
          ...
          const target = outputDir ? join3(outputDir, `${baseName}.${ext}`) : void 0;
...
var ACTIVE_CHANNELS_FILE2 = join3(tmpdir3(), "figma-active-channels.json");
async function readActiveChannels() {
  try {
    const raw = await readFile(ACTIVE_CHANNELS_FILE2, "utf8");
