...
var saveParams = {
  saveToFile: z.boolean().optional().describe(
...
server.tool(
  ...
  'List nodes under a root by type and/or name glob, one per line as `id:"name".TYPE @parent [x,y wxh]` \u2014 a flat, grep-friendly index of a subtree (the Figma analog of glob / `ls -R`). The `@parent` is the immediate container\'s short id (location without drawing the tree; pass it to get_node_info to see surroundings). The trailing `[x,y wxh]` is the node\'s ABSOLUTE bounding box (omitted with `bbox:false`, or when a node has no geometry). Filters: `type` (a node type, an array, or "*"/omit for any); `name` (a shell-style glob over the node\'s OWN name: `*` = any run, `?` = one char; omit for any \u2014 matches names only, not paths, since Figma names contain slashes); and `within` (an absolute rect \u2014 keep only nodes intersecting it, e.g. to glob a region; get its coords from a prior bbox). `root` is the node id to search under (default: current page); descends through every container regardless of match (any-depth search), with `depth` capping how deep. Ids (including `@parent`) are short counters (n0, n1, ...) \u2014 feed any straight into read/edit tools.',
  {
    ...
    name: z.string().optional().describe("Shell-style glob matched against each node's own name (* = any run, ? = one char). Case-insensitive. Omit to match any name."),
    type: z.union([z.string(), z.array(z.string())]).optional().describe('Node type filter: a single type (e.g. "TEXT"), an array (["TEXT","INSTANCE"]), or "*"/omit for any. Case-insensitive.'),
    depth: z.number().optional().describe("Max depth below root to descend (root's direct children = 1). Omit for unlimited."),
    bbox: z.boolean().optional().describe("Append each hit's absolute bounding box as [x,y wxh]. Default true; pass false to drop it and save tokens."),
    within: z.object({
      x: z.number(),
      y: z.number(),
      width: z.number(),
...
server.tool(
  ...
  "Edit node properties directly in the node model \u2014 the write-side twin of query_nodes/get_node_info, an Edit tool for Figma JSON instead of text. Pass `edits`: an array of `{nodeId, path, old?, new}`. `path` addresses one field the same way query_nodes does \u2014 dot for objects, `[i]` for an array index (e.g. `name`, `cornerRadius`, `fills[0].color`, `fills[0].opacity`); no `[*]` (a write needs one concrete target). `new` is the value to set: colors as `#RRGGBB` are converted to Figma's rgb 0-1, and whole objects/arrays are allowed (e.g. set `fills[0]` to a full paint). `old` is an OPTIONAL guard, exactly like the old_string in Edit \u2014 if given and it doesn't match the current value (colors compared as hex, numbers tolerantly), that one edit is rejected so you never blind-overwrite a stale read. Edits run in order and are INDEPENDENT: one failing \u2014 guard mismatch, read-only/derived prop, a type Figma rejects, font not loaded \u2014 records its Figma error and the rest still apply. The result lists each edit as `\u2713 id path: old \u2192 new` or `\u2717 id path: <error>`, so a failure tells you exactly what to fix. One call can touch many nodes (each edit names its own `nodeId`). Note width/height resize and `characters` loads the font. nodeId accepts short ids (n0, ...) or full Figma ids.",
  {
    edits: z.array(
      z.object({
        nodeId: z.string().describe("Node to edit. Short ids (n0, ...) or full Figma ids."),
        path: z.string().describe('Field path to write. Dot for objects, [i] for an array index. Same syntax as query_nodes, but no [*]. E.g. "name", "cornerRadius", "fills[0].color".'),
        old: z.any().optional().describe("Optional guard: expected current value (Edit-style). Colors as #RRGGBB. Mismatch rejects only this edit."),
        new: z.any().describe("Value to set. #RRGGBB \u2192 Figma rgb; numbers/strings/objects/arrays allowed.")
      })
